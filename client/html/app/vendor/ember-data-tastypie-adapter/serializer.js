...
var DjangoTastypieSerializer = DS.RESTSerializer.extend({
  ...
  */
  ...
  extractEmbeddedFromHasMany: function(store, embeddedKey, relationship, payload, config) {
    var serializer = store.serializerFor(relationship.type.typeKey),

    key = config.key ? config.key : this.keyForAttribute(embeddedKey);
  ...
  extractEmbeddedFromBelongsTo: function(store, embeddedKey, relationship, payload, config) {
    var serializer = store.serializerFor(relationship.type.typeKey),
